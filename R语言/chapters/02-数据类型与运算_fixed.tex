...
\begin{table}[ht]
  ...
  \toprule
    类别 & 常见符号 / 说明 \\
    \midrule
    算术运算符 & \texttt{+}, \texttt{-}, \texttt{*}, \texttt{/}, \texttt{\string^}, \texttt{\%\%}（取模），\texttt{\%/\%}（整数除法） \\
    关系运算符 & \textnormal{==}, \textnormal{!=}, \textnormal{>}, \textnormal{<}, \textnormal{>=}, \textnormal{<=} \\
    逻辑运算符 & 元素级：\textnormal{\&}、\verb!|!；短路（单值）：\textnormal{\&\&}、\textnormal{||}；取反：\textnormal{!} \\
    赋值运算 & 常用：\textnormal{<-}；也可使用：\textnormal{=}；右赋值：\textnormal{->}；全局赋值：\textnormal{<<-} \\
    \bottomrule
  \end{tabularx}
\end{table}
